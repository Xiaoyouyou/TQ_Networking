...
static QWSGzip *instan = nil;

@implementation QWSGzip
...
- (NSData *)gZipDataPressure:(NSData*)pressureData{
    ...
    zlibStreamStruct.next_in = (Bytef *)[pressureData bytes];
    zlibStreamStruct.avail_in = [pressureData length];
    ...
    NSMutableData *compressedData = [NSMutableData dataWithLength:[pressureData length] * 1.01 + 21];
...
- (NSData *)gzipDataExtract:(NSData *)extractData{
    ...
    if ([extractData length] == 0) return extractData;
    
    unsigned full_length = [extractData length];
    
    unsigned half_length = [extractData length] / 2;
    ...
    strm.next_in = (Bytef *)[extractData bytes];
    
    strm.avail_in = [extractData length];
